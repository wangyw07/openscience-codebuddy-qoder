...
export function SessionTurn(
  ...
  // Stream tokens into the summary pane as they arrive (typewriter-smoothed),
  // instead of waiting until the turn goes idle and dumping the full text.
  const streamedResponse = createTypewriter(() => response() ?? "")
  const messageDiffs = createMemo(() => message()?.summary?.diffs ?? emptyDiffs)
  const hasDiffs = createMemo(() => messageDiffs().length > 0)
  // Keep the final assistant text only in the summary pane (avoids duplicating
  // the same stream in the expanded steps list).
              ...
              >
                <Switch>
                  ...
                  <Match when={true}>
                    ...
                    {/* Response — visible while streaming so tokens paint live */}
                    <div class="sr-only" aria-live="polite">
                      {streamedResponse()}
                    </div>
                    <Show when={!!response() || (!working() && hasDiffs())}>
